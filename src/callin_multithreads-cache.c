...
int main(int argc, char* argv[])
{
  ...
  /* 
   * NOTE:  The call to CACHESETDIR is required to set the installation directory value
   * when linking using a shared library (.dll, or .so) on Windows, and some Unix platforms.
   */	
  rc=CACHESETDIR(shdir);
  printf("CACHESETDIR rc:%d\n",rc);
...
void *thread_main(void *tparam) {
  int *p = (int *)tparam;
  int termflag = CACHE_TTNONE|CACHE_TTNEVER;
  int timeout = 15;
  //CACHESTR pin;
  //CACHESTR pout;
  CACHESTR pusername;
  ...
  */
  ...
  __try {
#endif
  rc = CACHESECURESTART(&pusername,&ppassword,&pexename,termflag, timeout, NULL, NULL);
  if (rc) {
...
int runtest(int p) {
  ...
  // Do some dummy work
    ...
    RETURNIFERROR(rc);
    rc = CACHEPUSHINT(1); // Increment by 1
    RETURNIFERROR(rc);
    rc = CACHEGLOBALINCREMENT(0);
    RETURNIFERROR(rc);
    rc = CACHEPOPINT(&newId);
    RETURNIFERROR(rc);
    ...
    rc = CACHEPUSHGLOBAL(strlen((const char *)gloref), gloref);
    RETURNIFERROR(rc);
    rc = CACHEPUSHINT(newId); numargs++;
    RETURNIFERROR(rc);
    rc = CACHEPUSHSTR(strlen(data),data);
    RETURNIFERROR(rc);
    rc = CACHEGLOBALSET(numargs);
    RETURNIFERROR(rc);
...
void sigaction_handler(int signal, siginfo_t *si, void *arg)
{
  ...
  // Doc implies no need to call CACHEEND() myself.
